# CONTRIBUTING — ContentsX リポジトリ Git 運用ルール

> このドキュメントは **ContentsX（contentsx.jp）** リポジトリでチーム開発を行うための共通ルールです。
> 初めてこのリポジトリで作業する人は、最初に必ず一読してください。
> BizManga リポジトリにも同じ内容の `CONTRIBUTING.md` があります（差分はリポジトリ名・デプロイ先・ビルドBotだけ）。
