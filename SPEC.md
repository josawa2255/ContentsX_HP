...
**2026-07-07 イラスト改訂版へ差し替え**: 同キャラ・同構図の改訂版イラスト（元データ 2560×1440 PNG）から hero_bg 6ファイルを再生成して置換。寸法・ファイル名は従来と完全同一（PC 1672×941 / SP 1375×1144、各 avif/webp/png）のため HTML/CSS 変更なし。SP はキャラ中心が右寄り約66%になるよう元画像を crop 1731×1440(+229,0) してから縮小。変換は ffmpeg(lanczos) + cwebp(-q82) + avifenc(-q60)。
