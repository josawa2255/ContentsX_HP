/**
 * ContentsX i18n System
 * =====================
 * 日本語テキストをキーにした自動翻訳システム。
 * BizManga の bm-i18n.js と同じアーキテクチャ。
 *
 * 仕組み:
 ...
 *   data-ph-ja / data-ph-en の placeholder も対応
 */
(function () {
  'use strict';
  ...
  /* ============================================================
     設定
     ============================================================ */
  var STORAGE_KEY = 'cx-lang';

  var JSON_PATH = (function () {
    var scripts = document.getElementsByTagName('script');
    for (var i = 0; i < scripts.length; i++) {
      if (scripts[i].src && scripts[i].src.indexOf('i18n.js') !== -1 &&
          scripts[i].src.indexOf('bm-i18n') === -1) {
        return scripts[i].src.replace(/js\/i18n\.js.*$/, 'i18n/en.json');
      }
    }
    return 'i18n/en.json';
  })();
  ...
  function translateElement(el) {
    ...
    if (el.hasAttribute('data-ja') && el.hasAttribute('data-en')) {
      ...
      if (el.getAttribute('data-ja').indexOf('<') !== -1) {
        ...
        if (arrow) {
          // テキストノードだけ差し替え（arrow span を壊さない）
          var textNodes = getTextNodes(el);
          for (var i = 0; i < textNodes.length; i++) {
            if (textNodes[i].textContent.trim() && textNodes[i].parentNode === el) {
              textNodes[i].textContent = enAttr;
              break;
            }
          }
        } else {
          var textNodes2 = getTextNodes(el);
          if (textNodes2.length === 1) {
            el.textContent = enAttr;
          } else if (textNodes2.length > 1) {
    ...
    // ── data-ph-ja / data-ph-en: placeholder切替 ──
    if (el.hasAttribute && el.hasAttribute('data-ph-ja') && el.hasAttribute('data-ph-en')) {
      if (!originals.has(el)) {
        originals.set(el, { type: 'placeholder', value: el.placeholder });
      }
  ...
  function translateAll() {
    ...
    // data-ph-ja/data-ph-en 付き要素
    var phEls = document.querySelectorAll('[data-ph-ja][data-ph-en]');
    for (var p = 0; p < phEls.length; p++) {
      translateElement(phEls[p]);
    }
    // data-i18n-html 付き要素
  ...
  function switchLang(lang) {
    ...
    // ボタンの active 切替（ContentsX の header-lang-btn）
