...
(function() {
  ...
  function init() {
    ...
    var fragment = [
      ...
      '  // 頭頂部(0〜0.15)は完全固定。髪の毛先(0.18〜0.38)は弱く揺らす。',
      '  // 顔・胴体(0.38〜0.68)は固定。スカート(>0.68)は強く揺らす。',
      '  float hairArea  = smoothstep(0.18, 0.30, uv.y) * (1.0 - smoothstep(0.32, 0.42, uv.y));',
      '  float skirtArea = smoothstep(0.68, 0.98, uv.y);',
      '  float windStr   = skirtArea * uIntensity;',
      '  float hairStr   = hairArea * uIntensity * 0.55;   // 髪は控えめ',
      '',
      '  // --- ノイズ駆動の横風 ---',
      '  // 時間軸を主、Y軸を従にすることで「横方向にゆらぐ突風」を表現',
      '  // スカート用：低周波・大振幅（ゆったり大きく揺れる）',
      '  float skirtN = fbm(vec2(uv.y * 1.8, uTime * 0.55));',
      '  float skirtDisp = skirtN * 0.030 * windStr;',
      '',
      '  // 髪用：高周波・小振幅（毛先が細かくなびく）',
      '  float hairN = fbm(vec2(uv.y * 3.5, uTime * 1.1 + 17.3));',
      '  float hairDisp = hairN * 0.014 * hairStr;',
      '',
      '  float displacementX = skirtDisp + hairDisp;',
      '',
      '  // 横風のみ（縦揺れなし）',
      '  uv.x += displacementX;',
      '',
