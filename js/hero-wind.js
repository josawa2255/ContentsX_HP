...
(function() {
  ...
  function init() {
    ...
    var fragment = [
      ...
      '  // 顔・胸部（uv.y 0.18〜0.68）は完全デッドゾーン → 揺れない',
      '  // スカート相当（uv.y > 0.68）と 髪先端（uv.y < 0.18）のみ揺らす',
      '  float skirtArea = smoothstep(0.68, 0.98, uv.y);   // 下半身のみ',
      '  float hairArea  = smoothstep(0.18, 0.0, uv.y) * 0.4; // 髪先端のみ・控えめ',
